...
export default function ProductCard({ product } : { product: Product }) {
  return <div className="group relative w-50 h-123 rounded-lg border-2 border-gray-200 bg-white shadow-md hover:shadow-2xl hover:border-blue-500 transition-all duration-200 select-none">
    <Image src={product.main_image} alt={product.name} width={50} height={123} className="w-full aspect-5/4 rounded-t-md" />
    <Link href={`/product/${product.slug}`} className="block px-3">
      <section className="mt-2">
        <p className="font-medium max-h-12">{product.name}</p>
      ...
      <section className="flex flex-col gap-1.5">
        <p className="text-sm text-gray-500">Ngày bắt đầu: {product.created_at.toLocaleDateString("en-GB")}</p>
